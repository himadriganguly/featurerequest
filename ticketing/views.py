        ...
        if(request.method == 'POST'):
            ...
            if(form and form.is_valid()):
                ...
                try:
                    tmpform.save()
                    messages.success(
                        request, 'Your Data Has Been Saved. Thank You.')
    ...
    if (request.method == 'POST'):
        ...
        if request.is_ajax():
            ...
            if objs:
                ...
                response_data = {}
                response_data[
                    'msg'] = 'Priority cannot be update successfully!'
...
def data_presentation(request):
    ...
    if (request.method == 'GET'):
        ...
        for client in clients:
            client_feature_percent.update(
                {client.name: int((client.tickets.count() / total_features) * 100)})

        product_feature_total = {}
        for product in products:
            product_feature_total.update(
                {product.name: product.tickets.count()})

        target_date_total = {}
        for feature in features:
            s = "-"
            tempDate = (str(feature.targetdate.year), str(
                feature.targetdate.month), str(feature.targetdate.day))
            target_date_total.update({s.join(tempDate): FeatureRequest.objects.filter(
                targetdate=feature.targetdate).count()})
